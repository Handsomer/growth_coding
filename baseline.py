...
data=pd.merge(data,user_feature,on='uid',how='left') #data:aid特征，uid特征，aid,uid,label
...
def LGB_predict(train_x,train_y,test_x,res):#原始1500
    print("LGB test")
    clf = lgb.LGBMClassifier(
        boosting_type='gbdt', num_leaves=31, reg_alpha=0.0, reg_lambda=1,
    ...
    # train_y = train.pop('label')
    train_y["TrainScore"] = clf.predict_proba(train_x)[:,1]
    train_y["TrainScore"] = train_y["TrainScore"].apply(lambda x: float('%.6f' % x))
    train_y.to_csv('./data/lgbTrainScore.csv', index=False)
...
#nohup python baseline.py --gpu >baseline8000.txt 2>&1
#ps -aux|grep lr-3000.py
